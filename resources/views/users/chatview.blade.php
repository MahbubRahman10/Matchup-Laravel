		...
		<div class="col-md-12">	
			<div class="col-md-3" id="usersidebar">
				...
				<div class="sidebar-menu">
					<ul>
						...
						<li><a href="/profile/chat" class="sidebar-menu"><i class="fab fa-facebook-messenger"></i> Chat @if($chatstatus > 0)  <span id="chatstatus" class="chatstatus" style="background: #C32143; padding: 5px 10px; font-weight: bold; margin-left: 5px; border-radius: 10%;">{{ $chatstatus }}</span>  @endif</a></li>
			...
			<div class="col-md-9" id="usercontent">		
				...
				<div role="tabpanel" class="tab-pane active" id="usersetting">
					<h2><a href="/user/profile/{{ $destination->name }}/{{ $destination->id }}">{{ $destination->name }}</a></h2>
					<div class="row" style="margin-top: 0px; "><br>
						...
						<div class="col-sm-12 frame">
							<ul class="chat_ul">
								...
								@if (Auth::user()->id != $oldmessage->user_id)
								<li style="width:100%"> 
									<div class="msj macro"> 
										<div class="text text-l">
											<p> {{ $oldmessage->message }} </p>
											<p><small> {{ Carbon\Carbon::parse($oldmessage->created_at)->format('h:i A') }} </small></p>
										</div>
									</div> 
								</li>
							...
							<div>
								<div class="msj-rta macro">                        
									<div class="text text-r" style="background:whitesmoke !important;width: 100%;">
										<input class="mytext" placeholder="Type a message"/ data-id="{{ $destination->id }}">
									</div> 
					...
					if (value != '') {
						$.ajax({
							...
							success: function(data) {
								$('.chat_ul').prepend("<li style='width:100%'><div class='msj-rta macro'><div class='text text-l'><p> "+ value +" </p><p><small>" + date +  "</small></p></div></div></li>");
								$('.mytext').val('');
							}
						});
					}
	...
	<script type="text/javascript">
		$(document).ready(function() {
			setInterval(function(){   //calls click event after a certain time
				if ($('#chatstatus').hasClass('chatstatus') == true) {	
					var now = $('#chatstatus').html()
					now = parseInt(now) - 1;
					if (parseInt(now) == '0') {
						$('#chatstatus').removeClass('chatstatus')
			...
			function getmsg() {
				...
				$.ajax({
					type: 'get',
					url: '/profile/chat/getmsg',
					success: function(data) {
						for (var i = 0; i < data.length; i++) {
							$('.chat_ul').prepend("<li style='width:100%'><div class='msj macro'><div class='text text-l' style='padding-right: 0px;'><p> "+ data[i].message +" </p><p><small>" + data[i].created_at +  "</small></p></div></div></li>");
							$('.mytext').val('');
						}
